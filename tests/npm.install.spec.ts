...
describe(`install required packages`, () => {
  ...
  describe(`jest`, () => {
    ...
    describe(`babelification`, () => {
      [ "@babel/preset-env", "babel-jest" ].forEach(pkg => {
        it(`should install by default: ${ pkg }`, async () => {
          // Arrange
          const {
            name,
            where
          ...
          // Assert
          expect(shared.spawnModule)
            .toHaveInstalledDevDependency(pkg);
        });

        it(`should skip package when jest not required: ${pkg}`, async () => {
          // Arrange
          const {
            name,
            where
          ...
          // Assert
          expect(shared.spawnModule)
            .not.toHaveInstalledDevDependency(pkg);
        });
      });
    });
  });
  describe(`expect-even-more-jest`, () => {
    ...
    it(`should skip expect-even-more-jest on request`, async () => {
      ...
      await runTsBoot({
        name,
        where,
        includeExpectEvenMoreJest: false
      })
      // Assert
      expect(shared.spawnModule)
  ...
  it(`should install cross-env`, async () => {
    ...
    await runTsBoot({
      name,
      where,
      includeZarro: faker.random.boolean()
    });
    // Assert
    expect(shared.spawnModule)
